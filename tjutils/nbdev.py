...
from .utils import copy_template, copy_directory, merge_yaml
from nbdev.release import release_gh

# %% ../notebooks/01-nbdev.ipynb 2
def _poetry_command(*args):
    "Runs the `poetry` command line utility"
    subprocess.run(["poetry", *args])


def _black_command(*args):
    "Runs the `black` command line utility"
    subprocess.run(["black", *args])
...
def conform():
    """
    Conform nbdev, quarto, and poetry configuration. Infer information from poetry's
    `pyproject.toml` and the git repository information and generate:
    ...
    * a filled-in `pyproject.toml` for poetry.
    ...
    site_url = poetry.get("homepage", f"https://github.tjpalanca.com/{git_repo}")
    ...
    inferred = {
        ...
        "black_formatting": nbdev.get("black_formatting", "False"),
    ...
    if quarto_file.is_file():
        ...
        quarto_data["project"]["preview"] = {
            "host": quarto.get("host", "127.0.0.1"),
            "port": quarto.get("port", 8888),
            "browser": quarto.get("browser", False),
        }
        quarto_data["website"]["title"] = config["title"]
        quarto_data["website"]["description"] = config["description"]
        quarto_data["website"]["repo-branch"] = config["branch"]
        quarto_data["website"]["repo-url"] = git_url
        quarto_data["website"]["site-url"] = site_url
...
def export():
    ...
    _black_command(config["lib_path"])
...
def config():
    ...
    copy_template("nbdev/setup.cfg", "setup.cfg", append=True)


# %% ../notebooks/01-nbdev.ipynb 10
def theme():
    "Adds the TJ Palanca theme to quarto"
    config = get_config()
    # Make sure that
    quarto_file: Path = config["nbs_path"] / "_quarto.yml"
    if not quarto_file.is_file():
        nbdev_docs.__wrapped__()
    # Copy assets
    copy_directory("nbdev/theme", str(config["nbs_path"] / "theme"))
    # Merge in _quarto.yml
    merge_yaml("nbdev/_quarto.yml", str(quarto_file))
